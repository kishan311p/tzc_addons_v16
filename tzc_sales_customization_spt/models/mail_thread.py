            ...
            for record in self:
                ...
                if 'email' in data[0] and recipients:
                    for recipient in recipients:
                        self.with_context(recipient=recipient.name).env.ref('tzc_sales_customization_spt.partner_email_change_notify_admin_mail_template').sudo().send_mail(record.id,force_send=True,email_values={'recipient_ids':[(6,0,recipient.ids)]})
