...
class res_users(models.Model):
    ...
    def get_filtere_contact(self):
        ...
        if self.has_group('base.group_system') or self.has_group('dev_partner_access.group_partner_access_manager'):
            rec |= rec.search([(1,'=',1)])
        else:
            if self.has_group('base.group_user'):
                rec |= rec.search(['|',('id','=',self.partner_id.id),('is_user_internal','=',True)])
            if self.has_group('dev_partner_access.group_partner_access_salesperson'):
